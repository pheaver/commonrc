...
(defun darcsum-insert-file-line (title path end visible marked
  ...
  (let ((begin (point)))
    ...
    (insert
     ...
     (darcsum-add-face (concat path) 'darcsum-filename-face t)
     end)
    (if line-type
	(add-text-properties beg (point)
...
(defun darcsum-new-buffer (&optional subdir)
  ...
  (generate-new-buffer
   (concat "*darcs "
	   ...
	   (when subdir "/")
	   (when subdir file-relative-name subdir)
	   "*")))
...
(defun darcsum-reposition ()
  (unless (null (get-text-property (point) 'darcsum-line-type))
...
VIEW non-nil means open in View mode."
  ...
  (let ((file (darcsum-path (point)))
	(change-line
	 (and (eq 'change (get-text-property (point) 'darcsum-line-type))
	      (caadar (get-text-property (point) 'darcsum-line-change)))))
    (with-current-buffer
	(cond ((eq other 'dont-select)
      ...
      (recenter nil (display-buffer (current-buffer))))))
  ...
  (let (changeset)
    (dotimes (i (or arg 1))
      ...
      (while (progn
	       ...
	       (not (or (null (get-text-property (point) 'darcsum-line-type))
...
Return nil if there is no changeset matching NEXT-P."
  (let ((started (point)) changeset type)
    (if (catch 'no-next
	  (ignore
	   (dotimes (i (or arg 1))
	     (setq changeset (darcsum-changeset-at-point t))
...
(defun darcsum-display (data &optional look-for-adds)
  (with-current-buffer (darcsum-new-buffer)
    (darcsum-mode)
