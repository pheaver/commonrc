;;; cafeobj-mode.el -- emacs mode (sort of) for cafeobj
;;; Modified from maude-mode.el, by Philip Weaver
;;; Most of the features are made for maude, and thus do not work with cafeobj
;;; Feel free to add more features!
;;; The biggest problem right now is that in Maude practically EVERYTHING ends in .
;;;        but in cafeobj, only a few things (like eq) end in .
;;;        so things like vars, op, and pred don't get syntax hilighted

;;; Instructions:
;;; --------------------------------------------------------------------
;;; Put this file (cafeobj.el) in ~/lib/emacs
;;;
;;; Put the following in ~/.emacs:
;;;
;;;        (setq load-path (cons "~/lib/emacs" load-path))
;;;
;;;        (setq auto-mode-alist (cons '("\\.mod" . maude-mode) auto-mode-alist))
;;;        (defvar maude-cmd "cafeobj" "Defines the command line to call the Maude engine")
;;;        (require 'maude-mode)
...
;; 	 (list "\\[.*\\(\\<id:\\)\\s-+\\(\\w+\\).*\\]"   '(1 attribute-face prepend t)   '(2 attribute-value-face prepend t))
   ...
   (list  "\\(endom\\|endfth\\|endth\\|endoth\\|endv\\))"
	 ...
	 (list (concat (maude-flk-keyword "op\\|msg") maude-flk-term 
				 ...
				 '(5 maude-end-face prepend t)) ; .
	 ;; failed attempt to make pred a syntax hilighted keyword (Philip):
	 (list (concat (maude-flk-keyword "pred") maude-flk-term 
								 (maude-flk-keyword "to") maude-flk-term maude-flk-end)
				 '(1 font-lock-keyword-face prepend t) ; op
				 '(2 font-lock-function-name-face prepend t) ;
				 '(3 font-lock-keyword-face prepend t) ; to
...
;					      (looking-at (concat start-regexp "\\<\\(pred\\|vars\\|op\\|sort\\|subsort\\)s?\\>"))
;					      (looking-at (concat start-regexp "\\<\\(protecting\\|pr\\|extending\\|ex\\|including\\|inc\\)\\>"))
;					      )
;						(incf cur-indent (* 1 standard-indent))
;						(setq not-indented ()))
					 ((or (looking-at "\\s(")
								(looking-at "\\<if\\>"))
						(incf cur-indent 2))
...
;							(looking-at (concat start-regexp "\\<c?\\(pred\\|var\\|op\\|sort\\|subsort\\)s?\\>"))
;							)
